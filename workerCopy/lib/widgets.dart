...
class BrandLockup extends StatelessWidget {
  const BrandLockup({super.key, this.subtitle = 'Worker portal'});
...
class WorkerAvatar extends StatelessWidget {
  const WorkerAvatar({super.key, this.size = 46});
  final double size;
...
/// Keeps content readable on tablets / wide windows.
...
/// Quiet placeholder shown while data loads (calmer than a spinner).
class SkeletonBlock extends StatelessWidget {
  const SkeletonBlock({super.key, this.height = 76, this.radius = 18});
  final double height;
  final double radius;
...
/// Simple rounded level bar. Drawn by hand so it looks the same on every
/// Flutter version.
...
/// One supply (foam wash, disinfectant, water) with its level and state.
/// State is shown with an icon and words, not just color.
class SupplyGauge extends StatelessWidget {
  const SupplyGauge({super.key, required this.label, required this.level});
  final String label;
  final double level; // 0..100

  static Color colorFor(AppColors c, double v) {
    if (v <= 20) return c.danger;
    if (v <= 50) return c.signal;
    return c.success;
  }

  static String statusFor(double v) {
    if (v <= 20) return 'Low supply level';
    if (v <= 50) return 'Running low';
    return 'Normal supply level';
  }

  static IconData iconFor(double v) {
    if (v <= 20) return Icons.warning_amber_rounded;
    if (v <= 50) return Icons.error_outline_rounded;
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      label: '$label ${level.round()} percent. ${statusFor(level)}',
      child: ExcludeSemantics(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
              ...
              children: [
                Text(label, style: t.heading.copyWith(fontSize: 17)),
                Text(
                  '${level.round()}%',
            ...
            LevelBar(value: level / 100, color: color),
            const SizedBox(height: 8),
            Row(
              children: [
                Icon(iconFor(level), size: 16, color: color),
                const SizedBox(width: 6),
                Text(
                  statusFor(level),
                  style: t.caption.copyWith(
                    color: level <= 50 ? color : c.textMuted,
                    fontWeight: level <= 50 ? FontWeight.w700 : FontWeight.w500,
                  ),
                ),
              ],
            ),
